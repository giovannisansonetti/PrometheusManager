...
const TrashBinListElement = ({item, deletionDate, onClick}: TrashBinListElementProps) =>{
    return (
        <div>
            {item.isDeleted && (
            <div className="w-[80%] h-20 ml-[12%] top-5 border-1 border-[#27272a] rounded-md flex flex-row relative text-white mb-4 cursor-pointer" onClick={onClick}>
                ...
                <div className="hidden sm:absolute sm:left-[50%] sm:flex sm:items-center sm:h-full">{deletionDate}</div>
            </div>
            )}
        </div>
      )
...
export default TrashBinListElement